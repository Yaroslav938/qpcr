...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            # 1. Грузим сырой DF (с безопасной распаковкой на случай, если load_qpcr_csv возвращает 1 или 2 значения)
            ...
            if isinstance(load_result, tuple) and len(load_result) == 2:
                ...
                df_raw = load_result
                # Если функция не вернула флаг, вычисляем его сами
                is_qiagen = False
                if "Page 1" in df_raw.columns:
                    is_qiagen = True
                else:
                    try:
                        is_qiagen = df_raw.astype(str).apply(lambda col: col.str.contains("Page 1", na=False)).any().any()
                    except:
                        pass
