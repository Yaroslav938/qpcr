        ...
        if uploaded_file is not None:
            try:
                if uploaded_file.name.endswith(('.xlsx', '.xls')):
                    df_ready = pd.read_excel(uploaded_file)
                else:
                    df_ready, _ = robust_load_csv(uploaded_file)
                ...
                if st.button("Применить для анализа", type="primary"):
                    ...
                    st.session_state["raw_df"] = None # Сброс кривых
                    st.session_state["ds"] = None
                    st.session_state["ds_base"] = None
                    
                    st.success(f"Готово! Загружено {len(processed_df)} образцов. Можете переходить во вкладку 'Анализ (1 планшет)' или 'Калибровка'.")
            except Exception as e:
...
with tab_multi:
    ...
    m_data_mode = st.radio("Формат загружаемых файлов:", ["Сырые кривые амплификации", "Готовые таблицы (Ct)"], key="m_data_mode_tab8")
        ...
        if st.button("🚀 Запустить глобальный расчет", type="primary", key="m_run"):
            ...
            try:
                for i, file in enumerate(uploaded_files):
                    ...
                    else: # Готовые таблицы Ct
                        if file.name.endswith(('.xlsx', '.xls')):
                            df_ready_m = pd.read_excel(file)
                        else:
                            df_ready_m, _ = robust_load_csv(file)
                            
                        # Эвристика автоматического поиска колонок для множественных файлов
