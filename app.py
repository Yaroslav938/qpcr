...
with st.sidebar:
    ...
    if uploaded_file is not None:
        try:
            # 1. Грузим сырой DF
            df_raw, is_qiagen = load_qpcr_csv(uploaded_file)
            ...
            if is_qiagen:
                ...
                if st.button("🔄 Конвертировать в нормальный формат"):
                    try:
                        ...
                        # Ищем строку с заголовками (обычно содержит Page 1)
                        header_row_idx = None
                        ...
                        if header_row_idx is not None:
                            ...
                            # Переименовываем колонку циклов
            ...
            cycle_col = st.selectbox(
                ...
                options=["auto"] + list(df_raw.columns)
            )
            cycle_col_val = None if cycle_col == "auto" else cycle_col
            
            if st.button("Распознать структуру"):
                ds = build_dataset_from_raw(df_raw, cycle_col=cycle_col_val)
                st.session_state["ds"] = ds
                # Сброс последующих шагов
                st.session_state["ds_base"] = None
                st.session_state["batch_result"] = None
...
with tab_overview:
    ...
    if st.session_state["ds"] is not None:
        ...
        if len(selected_cols) < len(ds.sample_cols):
             new_ds = select_sample_columns(ds, selected_cols)
    ...
    if st.session_state["ds"] is not None:
        ...
        if st.button("Применить вычитание фона", type="primary"):
            ds_base = baseline_subtract(
                ds, 
                mode=base_mode, 
                base_cycles=list(range(base_cycles[0], base_cycles[1] + 1))
            )
...
with tab_fit_single:
    ...
    if ds_b is not None:
        ...
        if st.button("Подогнать кривую"):
            ...
            if res.success:
                st.success(f"Фиттинг успешен! Выбрана модель: **{res.model_name}**")
                ...
                with col2:
                    ...
                    # Смоделированная кривая (более гладкая)
                    x_smooth = np.linspace(min(x_raw), max(x_raw), 200)
                    y_smooth = res.predict(x_smooth)
                    fig.add_trace(go.Scatter(x=x_smooth, y=y_smooth, mode='lines', name=f'Модель {res.model_name}'))
                    
                    # Точка Ct (пересечение или cpD2)
                    if res.cpD2 is not None and not np.isnan(res.cpD2):
                        y_ct = res.predict(np.array([res.cpD2]))[0]
                        fig.add_trace(go.Scatter(
                            x=[res.cpD2], y=[y_ct], 
                            mode='markers', 
...
with tab_batch:
    ...
    if ds_b is not None:
        ...
        if st.button("Запустить Batch Fit", type="primary"):
            ...
            # В реальности batch_fit из qpcr_analysis работает синхронно и быстро.
            # Прогресс-бар здесь для красоты интерфейса.
            res_batch = batch_fit(ds_b, model_choice=batch_model)
            
            progress_bar.progress(100)
            status_text.text("Расчет завершён!")
            st.session_state["batch_result"] = res_batch
            
            st.success(f"Всего обработано: {res_batch.n_total}. Успешно: {res_batch.n_success}, Ошибок: {res_batch.n_failed}.")
                ...
                if final_table is not None:
                    ...
                    if not final_table.empty:
                        ...
                        with col2:
                            ...
                            if selected_gene in reference_genes:
                                ...
                                # Если целевой - строим Bar plot Fold Change со звездочками
                    ...
                    try:
                        ...
                        if not volcano_data.empty:
                            ...
                            # Добавляем таблицу-сводку под графиком
        ...
        if st.button("🚀 Запустить глобальный расчет", type="primary", key="multi_run"):
            ...
            try:
                for i, file in enumerate(uploaded_files):
                    ...
                    df_raw_m, is_q = load_qpcr_csv(file)
                    # Если формат Qiagen, нужна конвертация, но в мульти-режиме 
                    # мы предполагаем, что файлы уже нормального вида или функция load_qpcr_csv 
                    # справляется сама (в зависимости от вашей доработки).
                    # Если это голый экспорт, он прочитается нормально.
                    ds_m = build_dataset_from_raw(df_raw_m)
                    
                    # Применяем линейное вычитание фона как дефолт
                    ds_base_m = baseline_subtract(ds_m, mode="lin")
                    
                    # Пакетный фиттинг с автовыбором модели
                    fit_res_m = batch_fit(ds_base_m, model_choice="auto")
                ...
                grouped_table_m = group_replicates(combined_raw_table)
