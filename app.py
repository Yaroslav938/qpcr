...
def robust_load_csv(file_obj):
    """
    Умная загрузка CSV, которая правильно определяет разделитель 
    (защита от багов с запятыми в десятичных дробях).
    """
    file_obj.seek(0)
    ...
    # 2. Ищем строку с заголовками для определения разделителя
    header_line = lines[0]
    ...
    # 3. Выбор разделителя: считаем кол-во колонок в строке заголовка
    best_sep = ','
    max_cols = 0
    for s in [';', '\t', ',']:
...
def clean_dataframe_headers(df_raw):
    """
    Универсальная очистка "шапки" от текстового мусора
    и приведение колонки циклов в правильный числовой формат.
    """
    df_clean = df_raw.copy()
    
    # Ищем заголовок по названиям колонок
    header_found = False
    ...
    # Если заголовок съехал вниз, ищем его в строках
    if not header_found:
        try:
...
with tab_multi:
    ...
    if uploaded_files:
        col1, col2 = st.columns(2)
        with col1:
            ref_genes_str = st.text_input("Референсные гены (через запятую):", "actb1", key="multi_ref")
            ref_genes_multi = [g.strip() for g in ref_genes_str.split(",") if g.strip()]
        with col2:
            control_group_multi = st.text_input("Контрольная группа (Bio Group):", "0311 CTRL", key="multi_control")
        ...
        if st.button("🚀 Запустить глобальный расчет", type="primary", key="multi_run"):
            ...
            try:
                for i, file in enumerate(uploaded_files):
                    ...
                    # 1. Загружаем данные умным парсером
                    ...
                    try:
                        ...
                        raise ValueError(f"Ошибка в файле '{file.name}': Не удалось прочитать циклы амплификации. Подробности: {parse_e}")
