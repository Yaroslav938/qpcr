...
with tab_overview:
    ...
    if st.session_state["ds"] is not None:
        ...
        if len(selected_cols) < len(ds.sample_cols):
             # ИСПРАВЛЕНИЕ 1: Собираем датасет заново вручную без select_sample_columns
             new_ds = QPCRDataset(
                 df=ds.df.copy(), 
        ...
        if st.button("Подогнать кривую"):
            ...
            if res.success:
                ...
                with col2:
                    ...
                    # ИСПРАВЛЕНИЕ 2: Прямой вызов математической модели для сглаженной линии
