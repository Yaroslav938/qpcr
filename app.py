...
def init_state():
    ...
    if "multi_final_table" not in st.session_state:
        st.session_state["multi_final_table"] = None
    if "multi_bio_df" not in st.session_state:
        st.session_state["multi_bio_df"] = None

init_state()

# Класс-заглушка для совместимости готовых таблиц с логикой batch_fit
class DummyBatchResult:
    def __init__(self, table):
        self.table = table

def robust_load_csv(file_obj, header_row=0):
    ...
    header_line = lines[header_row] if len(lines) > header_row else lines[0]
    ...
    file_obj.seek(0)
    df = pd.read_csv(file_obj, sep=best_sep, header=header_row, encoding='utf-8')
    return df, False
...
# =====================================================================
# ДВИЖОК ГЛОБАЛЬНОГО АНАЛИЗА ЭКСПЕРИМЕНТА
# =====================================================================
...
# =====================================================================
def show_advanced_visualizations(final_table, bio_df, ref_genes, use_regression, widget_key="vis"):
    """Универсальная функция отрисовки графиков (Heatmap, Box-plot, Regression, Volcano)"""
    ...
    st.markdown("#### Анализ по отдельным генам (Экспрессия и Регрессия)")
    
    col_b1, col_b2 = st.columns([1, 3])
    with col_b1:
        selected_gene = st.selectbox("Выберите ген для детального графика:", options=final_table["Gene"].unique(), key=f"sel_gene_{widget_key}")
    with col_b2:
        fig_box = go.Figure()
        if selected_gene in ref_genes:
            ...
            fig_box.update_layout(title=f"Экспрессия гена {selected_gene} (Fold Change)", yaxis_title="Fold Change", template="plotly_white", yaxis=dict(rangemode='tozero'))
            st.plotly_chart(fig_box, use_container_width=True)
            
            gene_raw = bio_df[bio_df["Gene"] == selected_gene].dropna(subset=["Ref_Ct", "ct_mean"])
            if not gene_raw.empty:
                fig_reg = go.Figure()
                
                for grp in gene_raw["Treatment_Group"].unique():
                    grp_data = gene_raw[gene_raw["Treatment_Group"] == grp]
                    fig_reg.add_trace(go.Scatter(
                        x=grp_data["Ref_Ct"], y=grp_data["ct_mean"], 
                        mode='markers', name=grp,
                        text=grp_data["Bio_Sample"], hoverinfo="text+x+y",
                        marker=dict(size=10, line=dict(width=1, color='DarkSlateGrey'))
                    ))
                
                if use_regression and len(gene_raw) >= 3:
                    slope, intercept, r_value, p_value, std_err = stats.linregress(gene_raw["Ref_Ct"], gene_raw["ct_mean"])
                    x_min, x_max = gene_raw["Ref_Ct"].min(), gene_raw["Ref_Ct"].max()
                    margin = (x_max - x_min) * 0.1 if x_max != x_min else 1
                    x_range = np.array([x_min - margin, x_max + margin])
                    y_line = slope * x_range + intercept
                    
                    fig_reg.add_trace(go.Scatter(
                        x=x_range, y=y_line, 
                        mode='lines', 
                        name=f'Регрессия (b={slope:.2f}, R²={r_value**2:.2f})',
                        line=dict(color='black', dash='dash', width=2)
                    ))
                    title_reg = f"Глобальная регрессия (Wang et al.): {selected_gene} vs Ref_Index"
                else:
                    title_reg = f"Зависимость: {selected_gene} vs Ref_Index (без регрессии)"
                    
                fig_reg.update_layout(
                    title=title_reg,
                    xaxis_title="Reference Index Ct (Ref_Ct)",
                    yaxis_title=f"Target Gene Ct ({selected_gene})",
    ...
    try:
        ...
        if not volcano_data.empty:
            ...
            fig_volc.add_trace(go.Scatter(
                x=volcano_data[~sig_mask]["Log2_FC"], y=volcano_data[~sig_mask]["minus_log10_p"],
                mode='markers', marker=dict(color='grey', size=8, opacity=0.5),
                name='Not Sig', text=volcano_data[~sig_mask]["Gene"] + " (" + volcano_data[~sig_mask]["Group"] + ")",
                hoverinfo="text+x+y"
            ))
            fig_volc.add_trace(go.Scatter(
                x=volcano_data[sig_mask]["Log2_FC"], y=volcano_data[sig_mask]["minus_log10_p"],
                mode='markers', marker=dict(color='red', size=10),
                name='Significant (p<0.05, |log2FC|>1)', text=volcano_data[sig_mask]["Gene"] + " (" + volcano_data[sig_mask]["Group"] + ")",
                hoverinfo="text+x+y"
            ))
            fig_volc.add_hline(y=-np.log10(0.05), line_dash="dash", line_color="black")
            fig_volc.add_vline(x=1, line_dash="dash", line_color="black")
            fig_volc.add_vline(x=-1, line_dash="dash", line_color="black")
            fig_volc.update_layout(title="Volcano Plot (все опытные гены и группы)", xaxis_title="Log2 Fold Change", yaxis_title="-Log10 (P-value)", template="plotly_white")
            st.plotly_chart(fig_volc, use_container_width=True)
            
            st.markdown("#### Сводка значимых изменений")
            summary = final_table[final_table["P_value"] < 0.05]
            if not summary.empty:
                st.dataframe(summary[["Gene", "Group", "Log2_FC", "Fold_Change", "P_value"]].style.format({"Log2_FC": "{:.2f}", "Fold_Change": "{:.3f}", "P_value": "{:.4f}"}))
            else:
...
with st.sidebar:
    ...
    data_input_mode = st.radio(
        ...
        ["Сырые кривые амплификации", "Готовые таблицы (Ct)"]
    )
    
    if data_input_mode == "Сырые кривые амплификации":
        ...
        if uploaded_file is not None:
            try:
                ...
                if is_qiagen:
                    st.success("Файл успешно загружен и конвертирован из QIAGEN!")
                else:
    ...
    else: # Режим готовых таблиц Ct
        st.info("Загрузите таблицу с уже посчитанными Ct (CSV или Excel).")
        uploaded_file = st.file_uploader("Загрузите таблицу (CSV/Excel)", type=["csv", "txt", "xlsx", "xls"])
        
        if uploaded_file is not None:
            # Выбор строки с заголовками (важно для Excel с пустыми строками)
            header_row = st.number_input("С какой строки начинаются заголовки? (0 - первая)", min_value=0, value=0, step=1)
            
            try:
                if uploaded_file.name.endswith(('.xlsx', '.xls')):
                    df_ready = pd.read_excel(uploaded_file, header=header_row)
                else:
                    uploaded_file.seek(0)
                    df_ready = pd.read_csv(uploaded_file, header=header_row, sep=None, engine='python')
                ...
                table_format = st.radio(
                    "Структура таблицы:", 
                    ["Широкая (Гены в разных колонках)", "Длинная (Один столбец с Ct, один с Sample)"]
                )
                
                if table_format.startswith("Широкая"):
                    st.markdown("---")
                    st.markdown("**Назначение колонок (Широкий формат)**")
                    # Выбираем метаданные
                    meta_cols = st.multiselect(
                        "1. Колонки метаданных (Среда, Организм, День и т.д.):", 
                        options=df_ready.columns
                    )
                    # Выбираем гены
                    gene_cols = st.multiselect(
                        "2. Колонки генов (содержат значения Ct):", 
                        options=[c for c in df_ready.columns if c not in meta_cols]
                    )
                    
                    if st.button("Применить для анализа", type="primary"):
                        if not meta_cols or not gene_cols:
                            st.warning("Выберите хотя бы одну колонку метаданных и один ген!")
                        else:
                            # 1. "Плавим" таблицу из Широкого в Длинный формат
                            melted = pd.melt(df_ready, id_vars=meta_cols, value_vars=gene_cols, var_name="Gene", value_name="Ct")
                            
                            # 2. Очищаем Ct (заменяем запятые, убираем прочерки)
                            melted["Ct"] = melted["Ct"].astype(str).str.replace(',', '.').str.replace('-', 'NaN')
                            melted["Ct_cpD2"] = pd.to_numeric(melted["Ct"], errors='coerce')
                            
                            # 3. Убираем пустые Ct
                            melted = melted.dropna(subset=["Ct_cpD2"])
                            
                            # 4. Формируем единое имя группы из метаданных
                            def make_bio_group(row):
                                vals = []
                                for c in meta_cols:
                                    val = row[c]
                                    # Игнорируем пустые/NaN ячейки, чтобы не было "nan" в названии
                                    if pd.notna(val) and str(val).strip().lower() not in ['nan', 'none', '']:
                                        vals.append(str(val).strip())
                                return "_".join(vals)
                                
                            melted["bio_group"] = melted.apply(make_bio_group, axis=1)
                            
                            # 5. Формируем финальное имя sample (строго "Ген Биологическая_Группа")
                            melted["sample"] = melted["Gene"] + " " + melted["bio_group"]
                            melted["Efficiency_cpD2"] = 2.0
                            
                            # 6. Сохраняем в сессию
                            st.session_state["batch_result"] = DummyBatchResult(melted[["sample", "Ct_cpD2", "Efficiency_cpD2"]])
                            st.session_state["raw_df"] = None
                            st.session_state["ds"] = None
                            st.session_state["ds_base"] = None
                            
                            st.success(f"Готово! Извлечено {len(melted)} валидных значений Ct. Перейдите во вкладку 'Анализ (1 планшет)'.")

                else:
                    st.markdown("---")
                    st.markdown("**Назначение колонок (Длинный формат)**")
                    col_sample = st.selectbox("Названия образцов (Sample):", options=df_ready.columns, index=0)
                    col_ct = st.selectbox("Значения Ct:", options=df_ready.columns, index=1 if len(df_ready.columns)>1 else 0)
                    
                    has_eff = st.checkbox("В таблице есть эффективность (E)?")
                    col_eff = st.selectbox("Значения Эффективности:", options=df_ready.columns) if has_eff else None
                    
                    if st.button("Применить для анализа", type="primary"):
                        processed_df = pd.DataFrame()
                        processed_df["sample"] = df_ready[col_sample].astype(str)
                        processed_df["Ct_cpD2"] = pd.to_numeric(df_ready[col_ct].astype(str).str.replace(',', '.'), errors='coerce')
                        
                        if has_eff and col_eff:
                            processed_df["Efficiency_cpD2"] = pd.to_numeric(df_ready[col_eff].astype(str).str.replace(',', '.'), errors='coerce')
                        else:
                            processed_df["Efficiency_cpD2"] = 2.0
                            
                        st.session_state["batch_result"] = DummyBatchResult(processed_df)
                        st.session_state["raw_df"] = None
                        st.session_state["ds"] = None
                        st.session_state["ds_base"] = None
                        
                        st.success(f"Готово! Загружено {len(processed_df)} образцов.")
            except Exception as e:
...
with tab_overview:
    ...
    if st.session_state["raw_df"] is not None:
        ...
        if st.session_state["batch_result"] is not None:
            st.info("Работа идет в режиме готовой таблицы Ct. Графики кривых недоступны.")
        else:
...
with tab_baseline:
    ...
    if st.session_state["raw_df"] is None and st.session_state["batch_result"] is not None:
         st.info("Этот шаг не требуется для готовых таблиц Ct.")
    else:
...
with tab_batch:
    ...
    if st.session_state["raw_df"] is None and st.session_state["batch_result"] is not None:
         st.info("Этот шаг не требуется для готовых таблиц Ct. Можете переходить на следующие вкладки.")
    else:
    ...
    if st.session_state.get("batch_result") is not None:
        with st.expander("Показать таблицу результатов из памяти", expanded=True if st.session_state["raw_df"] is None else False):
...
with tab_calib:
    ...
    st.markdown("Оцените эффективность ПЦР ($Ct$ от $\log_{10}$ разведения).")
    ...
    if batch_res is not None:
        ...
        st.info("Сначала выполните Пакетный фиттинг или загрузите готовую таблицу Ct.")
...
with tab_experiment:
    ...
    if batch_res is not None:
        ...
        st.markdown("### Шаг 1. Группировка и выбор параметров")
        
        temp_names = table["sample"].apply(lambda x: re.sub(r'(\s*\[\d+\]|\s*_\d+|\s*\.\d+|\s*-\d+)\s*$', '', str(x)).strip())
        temp_genes = sorted(temp_names.apply(lambda x: str(x).split(" ")[0]).unique())
        temp_bio_groups = temp_names.apply(lambda x: " ".join(str(x).split(" ")[1:])).unique()
        temp_treatments = [re.sub(r'[\s\-_]*\d+$', '', x).strip() for x in temp_bio_groups]
        temp_treatments = sorted(list(set([x if x else y for x, y in zip(temp_treatments, temp_bio_groups)])))

        col1, col2 = st.columns(2)
        with col1:
            ref_genes_list = st.multiselect("Выберите референсные гены:", options=temp_genes)
        with col2:
            control_group_str = st.selectbox("Выберите контрольную группу:", options=[""] + temp_treatments)
        ...
        if st.button("🚀 Рассчитать экспрессию", key="s_calc", type="primary"):
            if not ref_genes_list:
                st.error("Пожалуйста, выберите хотя бы один референсный ген.")
            elif not control_group_str:
                st.error("Пожалуйста, выберите контрольную группу.")
            else:
                try:
        ...
        if final_table is not None and not final_table.empty:
            show_advanced_visualizations(final_table, bio_df, ref_genes_list, use_regression, widget_key="tab7")
    else:
        st.info("Для анализа необходимо сначала загрузить данные.")
...
with tab_multi:
    ...
    m_data_mode = st.radio(
        "Формат загружаемых файлов:", 
        ["Сырые кривые амплификации", "Готовые таблицы (Длинные)", "Готовые таблицы (Широкие - Гены в колонках)"], 
        key="m_data_mode_tab8"
    )
    
    st.info("Загрузите все файлы экспериментов (3 точки). Программа соберет их вместе.")

    uploaded_files = st.file_uploader(
        "Загрузите файлы экспорта (CSV/txt/rex/xlsx)", 
        type=["csv", "txt", "rex", "xlsx", "xls"], 
    ...
    if uploaded_files:
        ...
        # Если широкие таблицы - нужно определить колонки метаданных и генов по первому файлу
        m_meta_cols, m_gene_cols, m_header_row = [], [], 0
        if m_data_mode == "Готовые таблицы (Широкие - Гены в колонках)":
            st.warning("Будет использована структура ПЕРВОГО файла для всех остальных. Убедитесь, что колонки совпадают.")
            first_file = uploaded_files[0]
            first_file.seek(0)
            m_header_row = st.number_input("Строка с заголовками (0 - первая):", min_value=0, value=0, step=1, key="m_head_multi")
            try:
                if first_file.name.endswith(('.xlsx', '.xls')):
                    df_preview = pd.read_excel(first_file, header=m_header_row)
                else:
                    df_preview = pd.read_csv(first_file, header=m_header_row, sep=None, engine='python')
                    
                m_meta_cols = st.multiselect("Колонки метаданных для всех файлов:", options=df_preview.columns, key="m_meta")
                m_gene_cols = st.multiselect("Колонки генов для всех файлов:", options=[c for c in df_preview.columns if c not in m_meta_cols], key="m_gene")
            except Exception as e:
        ...
        if st.button("🚀 Запустить глобальный расчет", type="primary", key="m_run"):
            ...
            try:
                for i, file in enumerate(uploaded_files):
                    ...
                    if m_data_mode == "Сырые кривые амплификации":
                    ...
                    elif m_data_mode == "Готовые таблицы (Длинные)":
                        if file.name.endswith(('.xlsx', '.xls')):
                            df_ready_m = pd.read_excel(file)
                        else:
                            df_ready_m, _ = robust_load_csv(file)
                            
                        cols = df_ready_m.columns
                        sample_col = next((c for c in cols if "sample" in str(c).lower() or "name" in str(c).lower() or "образец" in str(c).lower()), cols[0])
                        ct_col = next((c for c in cols if "ct" in str(c).lower() or "cq" in str(c).lower() or "cp" in str(c).lower()), cols[1] if len(cols)>1 else cols[0])
                        eff_col = next((c for c in cols if "eff" in str(c).lower() or "e" == str(c).lower()), None)
                        
                        temp_df = pd.DataFrame()
                        temp_df["sample"] = df_ready_m[sample_col].astype(str)
                        temp_df["Ct_cpD2"] = pd.to_numeric(df_ready_m[ct_col].astype(str).str.replace(',', '.'), errors='coerce')
                        if eff_col:
                            temp_df["Efficiency_cpD2"] = pd.to_numeric(df_ready_m[eff_col].astype(str).str.replace(',', '.'), errors='coerce')
                        else:
                            temp_df["Efficiency_cpD2"] = 2.0
                            
                        all_raw_tables.append(temp_df)
                        
                    elif m_data_mode == "Готовые таблицы (Широкие - Гены в колонках)":
                        if file.name.endswith(('.xlsx', '.xls')):
                            df_ready_m = pd.read_excel(file, header=m_header_row)
                        else:
                            df_ready_m = pd.read_csv(file, header=m_header_row, sep=None, engine='python')
                            
                        melted = pd.melt(df_ready_m, id_vars=m_meta_cols, value_vars=m_gene_cols, var_name="Gene", value_name="Ct")
                        melted["Ct"] = melted["Ct"].astype(str).str.replace(',', '.').str.replace('-', 'NaN')
                        melted["Ct_cpD2"] = pd.to_numeric(melted["Ct"], errors='coerce')
                        melted = melted.dropna(subset=["Ct_cpD2"])
                        ...
                        all_raw_tables.append(melted[["sample", "Ct_cpD2", "Efficiency_cpD2"]])
