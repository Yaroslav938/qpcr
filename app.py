    ...
    if uploaded_file is not None:
        try:
            ...
            if isinstance(load_result, tuple) and len(load_result) == 2:
                ...
                # Умная проверка: сырой ли это файл с метаданными сверху?
                ...
                if "Excel Raw Data Export" in first_col_name or "QIAGEN" in first_col_name or "Unnamed" in first_col_name:
                    # Если первая колонка называется странно, ищем внутри Page 1
                    try:
                        if df_raw.astype(str).apply(lambda col: col.str.contains("Page 1", na=False)).any().any():
                            is_qiagen = True
                    except:
                        pass
                # Если же колонка УЖЕ называется Page 1 или Cycle, это нормальная матрица
...
with tab_baseline:
    ...
    if st.session_state["ds"] is not None:
        ...
        if st.button("Применить вычитание фона", type="primary"):
            # ИСПРАВЛЕНИЕ: Передача start_cycle и end_cycle явно
...
with tab_batch:
    ...
    if ds_b is not None:
        ...
        if st.button("Запустить Batch Fit", type="primary"):
            ...
            # Подсчет результатов по таблице
        ...
        if st.button("🚀 Запустить глобальный расчет", type="primary", key="multi_run"):
            ...
            try:
                for i, file in enumerate(uploaded_files):
                    ...
                    # Автоматическая конвертация QIAGEN на лету для мульти-мода!
                    ...
                    # ИСПРАВЛЕНИЕ: Передача start_cycle=1.0 и end_cycle=10.0 для мульти-планшета
