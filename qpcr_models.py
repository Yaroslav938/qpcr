...
для обхода ограничений плавающей точки и эффекта дискретной сетки.
"""
...
def l4_model(x: np.ndarray, b: float, c: float, d: float, e: float) -> np.ndarray:
    ...
    c_safe = max(c, 1e-5)
    return d + (e - d) / (1.0 + np.exp(b * (np.log(x_safe) - np.log(c_safe))))
...
def l5_model(x: np.ndarray, b: float, c: float, d: float, e: float, f: float) -> np.ndarray:
    ...
    x_safe = np.clip(x, 1e-5, None)
    c_safe = max(c, 1e-5)
    return d + (e - d) / (1.0 + np.exp(b * (np.log(x_safe) - np.log(c_safe))))**f
...
def _find_peak_zooming(func: Callable, x_min: float, x_max: float, order: int = 2, iters: int = 3, pts: int = 100) -> float:
    """
    Итеративный поиск максимума 1-й или 2-й производной методом сужающегося окна (Zooming).
    Гарантирует высочайшую точность (до 6-8 знаков) при минимальных вычислениях (300 точек),
    избегая переполнения плавающей точки от аналитических уравнений.
    
    order = 1 (для cpD1), order = 2 (для cpD2/Ct)
    """
    curr_min = x_min
    curr_max = x_max
    peak_x = None
    ...
    for _ in range(iters):
        x_eval = np.linspace(curr_min, curr_max, pts)
        y_eval = func(x_eval)
        
        # Численное дифференцирование на текущем отрезке
        dy = np.gradient(y_eval, x_eval)
        if order == 1:
            target_array = dy
        else:
            d2y = np.gradient(dy, x_eval)
            target_array = d2y
            
        idx_max = int(np.argmax(target_array))
        peak_x = x_eval[idx_max]
        
        # Сужаем окно для следующей итерации (по 1 шагу влево и вправо от найденного пика)
        step = (curr_max - curr_min) / (pts - 1)
        curr_min = max(x_min, peak_x - step)
        curr_max = min(x_max, peak_x + step)
...
def fit_curve_l4(x_raw, y_raw) -> FitResult:
    ...
    try:
        with warnings.catch_warnings():
            ...
            popt, pcov = curve_fit(l4_model, x, y, p0=p0, maxfev=10000)
        ...
        # Новый итеративный поиск пиков (zooming)
...
def fit_curve_l5(x_raw, y_raw) -> FitResult:
    ...
    try:
        with warnings.catch_warnings():
            warnings.simplefilter("ignore")
            # Ограничиваем параметр асимметрии f, чтобы он не улетал в бесконечность
            bounds = ([-np.inf, 1e-5, -np.inf, -np.inf, 0.01], [np.inf, np.inf, np.inf, np.inf, 100.0])
            popt, pcov = curve_fit(l5_model, x, y, p0=p0, bounds=bounds, maxfev=10000)
